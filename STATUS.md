...
M2.0 ✅ (kararlar) · M2.1 ✅ (görsel + yapısal temel) · **M2.2 ✅** — grace şeması + parent_receipt_id zemini + bootstrap dönem + §17 timezone + draft kaldırma + yönlendirme trigger + storage (receipts bucket private + policy) canlıda. 3c kararı kapandı (mekanik M2.3'te). **Sıra: M2.3 saha ekranı.** Bu commit = chrome-hizalama: saha nav→4 (Ana/Dönem/Ara/Mesajlar) + header→spec (avatar-sol/proje-orta/badge-sağ/5-menü) + placeholder-disiplini kuralı. dept/muhasebe nav farklı layout → TODO-SPEC (M2.4/M2.5). NOT: M2.1 nav/header B4 stub'ları spec'e uymuyordu (işaretsizdi); bu commit saha+header'ı hizaladı. C1 gövdesi indi: src/app/saha/ (saha-screen router + saha-home-screen) — FİŞ TARA diski (G6 logo açık slot + oturum-bazlı açılış pulse) + Galeri/Belgesiz görünür butonlar; **uzun-bas çıkarıldı** (görünür buton > gizli submenu; EKRAN-SAHA §2 güncellendi). Disk/Galeri/Belgesiz tıklanır ama akış "yakında" (C2/C3). **Scroll-altı veri kartları (bütçe/kategori/son-3-fiş/avans) C1'de BASILMADI** — sahte placeholder yerine gerçek veriyle gelecek (son-3-fiş C2 sonrası; avans/bütçe kendi feature'ı); eksik değil, kasıt (placeholder-disiplini). C2a (receipt_no kolonu) + C2b (fiş giriş akışı) indi: kamera/galeri→foto→storage upload (projectId/receiptId/receipt.ext, bucket private)→form (Satıcı·Tutar·KDV·Tarih·Fiş No·Kategori·Açıklama, elle)→submitted INSERT (trigger kuyruğa + dept_id). Storage path canlı RLS ile uyumlu (project_id()=[1], owner=auth.uid()). owner=auth.uid() + storage uçtan uca CANLIDA test edilecek (foto çek→kaydet→fiş satırı + storage objesi teyidi). Sıra: C2c kategori ek-panelleri (Ulaşım/Yemek/Konaklama/Kiralama — §6, limit değerleri bekliyor) + C5 dönem ekranı. Ertelenenler (işaretli): dijital imza (G2 slot) · açıklama mikrofonu + GİB-doğrula (Faz 2) · domain.ts tip tamamlama (receipt_no/vat_amount/receipt_date — C5/C6'da fiş tipli okununca).
...
## Son Session (3 Haziran 2026 — B grant + 3c mekaniği + build fix)
HEAD `5dd4c94`. Tamamlananlar (hepsi origin'den bağımsız doğrulandı, build dahil):
- **authenticated YAZMA izni — KARAR B ✅.** Önce 21 public tabloda RLS-açık + her tabloda ≥1 policy teyit edildi (salt-okunur). Sonra Faz-1 yazma seti canlıya uygulandı (Engin SQL Editor) + repo senkron (SUPABASE-RLS.sql + sql/full-rebuild.sql): `GRANT INSERT` 16 tablo, `GRANT UPDATE` 15 tablo — RLS komut haritasına birebir (blanket GRANT ALL DEĞİL); DELETE yok; advance_log/approval_log/projects yazma almaz; notifications/period_closings yalnız UPDATE. ALTER DEFAULT PRIVILEGES authenticated'a yalnız SELECT → yeni tablo otomatik yazma izni almaz.
- **3c düzeltme mekaniği (Model B) ✅** (commit `68a5c77`). receipts'e `correction_requested` (bool) + `correction_note` (text); saha düzeltme RLS penceresi `receipts_saha_correction` (user_id=auth.uid() AND correction_requested=true AND dönem open/closing); kolon-disiplini trigger `fn_receipt_correction_discipline` / `trg_receipt_correction_discipline` (saha penceresinde proje/dönem/sahip/dept/parent/created_at/is_late_entry/gib_qr_verified değişemez; tamamlanınca correction_requested=false + note=NULL + status='submitted'). Canlı + repo (SUPABASE-RLS.sql + full-rebuild.sql) + UI: reviewer-screen "Düzeltme İste" (fonksiyonel), saha-home düzeltme banner + receipt-correction-screen "Düzelt ve Gönder", domain.ts'e correction alanları + ertelenen vat_amount/receipt_date/receipt_no. Saha düzeltme akışı canlı test edildi ✅ (reviewer SQL ile taklit; correction true→false + status→submitted doğrulandı).
- **Build fix** (commit `5dd4c94`): receipt-service.ts'de iki `Receipt[]` cast'i tsc -b'de TS2352 verdi → `as unknown as Receipt[]`. npm run build yeşil (Opus origin'den build edip doğruladı), Vercel prod yeşil.
- **DERS:** Sonnet "tsc --noEmit temiz" diye YANLIŞ raporlayabilir (build-mode `tsc -b` farklı yakalar). Sonnet checklist'inde artık `npm run build` (= tsc -b && vite build) zorunlu; "✓ built" görmeden commit yok. (CLAUDE.md'ye işlenecek — açık borç.)
- **Açık (next'e taşındı):** reviewer **Onayla/Reddet** = stub (sadece toast). `submitted→dept_pending` routing UI/trigger YOK → reviewer kuyruğu boş kalıyor (test için fiş elle dept_pending'e alındı). Onboarding / davet UI YOK.
...
1. **GAP AUDIT (ilk iş).** "Sıra" şimdiye dek reaktif bir özellik listesiydi, uçtan-uca plan değildi; bu oturumda 3 boşluk iş gerekince ortaya çıktı (onboarding/davet UI, `submitted→dept_pending` routing, reviewer Onayla/Reddet). Özellik yığmadan ÖNCE uçtan-uca tarama yap: onboarding → proje → fiş gir → routing → incele/onayla/reddet/düzelt → dönem kapama → rapor. Eksiklerin TAM listesini çıkar → gerçek sıralı backlog (IS-SIRASI.md güncelle).
2. **Davet / rol-atama akışı (tam feature — KARAR: şimdi yapılacak).** Backend hazır (invitations + profiles + set-claims/clear-claims + user_role/user_dept_id/project_id helper'ları). UI YOK. Karar: token-link tabanlı; kişi KENDİ şifresini koyar (Muhasebe şifre üretmez), ama Muhasebe proje içinde her hesaba/kayda tam erişim + müdahale yetkisine sahip (RLS ile garanti). v1: davet açan yalnız Muhasebe, e-posta entegrasyonu sonra.
   - **Aşama 1 — Davet oluşturma:** "Davet Et" ekranı (ad/soyad/e-posta/rol/dept) → token üret → invitations satırı (status=pending, expires_at) + kopyalanabilir davet linki. RLS: yalnız muhasebe insert.
   - **Aşama 2 — Kabul + kayıt:** linkteki token → signup ekranı (e-posta/şifre) → `accept-invitation` SECURITY DEFINER fn (token doğrula → profili davetin rol/dept'iyle oluştur → davet accepted) → claims set → giriş. Rol kullanıcıdan DEĞİL davetten gelir. Auth routing: token linki, giriş yapılmamışken accept ekranına gitmeli.
   - **Aşama 3 (sonra):** üye yönetimi (liste, revoke/arşiv = membership_status), Dept→Saha incelikleri.
3. `submitted→dept_pending` routing (reviewer kuyruğunu doldurur) + reviewer Onayla/Reddet (stub'ı gerçek statü geçişine + approval_log'a bağla) — gap audit önceliklendirmesine göre.
4. (Beklemede) C2c kategori ek-panelleri (§6 limit değerleri Engin'den) · C5 dönem ekranı · C3 belgesiz.
Detaylı sıra: docs/IS-SIRASI.md (gap audit'ten sonra güncellenmiş hâli).
...
| SUPABASE-RLS.sql | güncel | fiş insert/delete dönem IN('open','closing') · receipts_insert saha+dept + giriş-statüsü guard · saha update/delete YOK · yönlendirme trigger (fn_route_receipt/trg_route_receipt); VARSAYIMLAR trigger sayısı 3 · §18 STORAGE receipts policy (insert saha/dept · select muhasebe+sahibi+dept) · 3c düzeltme (correction_requested/note + receipts_saha_correction RLS + fn_receipt_correction_discipline trigger) · Faz-1 authenticated yazma grantları (INSERT 16 / UPDATE 15) |
| SUPABASE-FUNCTIONS.sql | güncel | clear_user_claims RPC (yönlendirme trigger RLS dosyasında yaşıyor) |
| sql/full-rebuild.sql | güncel | grace + parent_receipt_id + draft-kaldırma + yönlendirme trigger + STORAGE receipts policy + receipt_no yansımış · 3c düzeltme (correction_requested/note + receipts_saha_correction RLS + fn_receipt_correction_discipline trigger) · Faz-1 authenticated yazma grantları (INSERT 16 / UPDATE 15) |
...
| src/ | M2.3 C2b | C1 + C2b fiş giriş: src/app/saha/receipt-entry-screen (foto önizleme + elle form §4: Satıcı/Tutar/KDV/Tarih/Fiş No/Kategori/Açıklama → submitted INSERT) · shared/supabase/receipt-service (getCategories + createReceipt = storage upload + INSERT, kimlik claim'den) · saha-home disk/Galeri → gizli input (capture/galeri) → entry ekranı; Belgesiz hâlâ "yakında" (C3) · kategori ek-panelleri (§6) + imza/mic sonraya · dept/muhasebe nav = TODO-SPEC · reviewer-screen (Düzeltme İste fonksiyonel; Onayla/Reddet stub) · receipt-correction-screen · saha-home düzeltme banner · domain.ts correction alanları |
| README.md | minimal | G6 todo |
| STATUS.md | güncel | 3 Haz: B grant + 3c mekaniği + build fix işlendi; next = gap audit + davet/rol |
...
- [x] authenticated Faz-1 yazma grantları (Karar B): 21 tablo RLS-açık teyidi + GRANT INSERT 16 / UPDATE 15, canlı + repo senkron (3 Haz)
- [x] 3c düzeltme mekaniği (Model B): correction_requested/note + saha-pencere RLS + kolon-disiplini trigger + UI; saha tarafı canlı test — 68a5c77 (3 Haz)
- [x] Build fix TS2352 (as unknown as Receipt[]) — 5dd4c94, Vercel yeşil (3 Haz)
