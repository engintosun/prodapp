...
- Kullanıcıya MESAJ OLARAK gösterilen metin TAM Türkçe karakterli yazılır: JSX metni, `placeholder`, `title`, `aria-label`, `setError()` içeriği ve Supabase hatasını kullanıcıya taşıyan `error.message` zincirinin kaynağı (SQL `raise exception` metinleri). ASCII'ye düşürme YASAK. Ölçüt konum değil GÖRÜNÜRLÜKTÜR: motorun kendi iç şart hataları (`src/shared/cfe/`), `console.warn` metinleri ve test kurgusundaki metinler kullanıcıya mesaj olarak çıkmaz — ASCII kalırlar. Kod tarafı da ASCII kalır: değişken adı, kolon anahtarı, rota yolu, enum değeri, test id, kod yorumu. KAPI: `.claude/hooks/gate.sh` eklenen satırlarda bilinen ASCII köklerini büyük/küçük harf ayırmadan arar — `.md` dosyalarında ve `src/**/*.ts(x)` içinde (`*.test.*` ve `src/shared/cfe/` hariç; yorumla başlayan satırlar atlanır). Liste SABİTTİR: listede olmayan yeni bir kelime yanlış yazılırsa kapı susar, kural yine elle uygulanır. Kaynak: TD-32 — prompt ASCII yazıldığı için ekrana ASCII metin çıkmıştı.
