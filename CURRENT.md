...
- ÜRETİM KAYITLARI DURAĞI kod VE şema CANLIDA (bkz. Milestone 6 Eylül). Şema (ajans/menajer haneleri) `db push` ile uygulandı, dört kolon + `budget_cost_objects_temsilci_haneleri_check` kısıtı salt-okuma sorguyla doğrulandı. Ekranda gerçek veriyle elle deneme (tarayıcı üzerinden) HENÜZ YAPILMADI.
- Migration 20260901130000'den 20260906120000'e kadar (KART 1600 M1/M2/M3, K-B iptali, MÜHÜR-3 dilim 1-2, Üretim Kayıtları temsilci haneleri dahil) CANLIDA. Kod: 330/330 test. Build geçer, eslint 0 hata. Originde tek dal: main.
...
**MADDE 1: ÜRETİM KAYITLARI — TARAYICIDA ELLE DOĞRULAMA.** Kod ve şema CANLIDA (bkz. Milestone/Durum), ama bugüne kadarki doğrulama yalnız test paketi + salt-okuma SQL sorgusuydu. Sıradaki oturumun ilk işi: dev sunucu açılır, gerçek bir projede Üretim Kayıtları durağına girilir, Oyuncular masası ve listesi elle denenir (kişi ekleme, rol/oyuncu/görev, ajans/menajer tiki + ad hanesi açılış-kapanışı), sonra karttan açılan salt-okunur pano ile çapraz kontrol edilir.
