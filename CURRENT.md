...
- KRITIK ACIK: butce UI/servis yok (Dilim 2b+); uyari/anomali motoru yok (kural var, kod yok). cost_object sema + butce runtime dilimi kod-tarafini ACAR (kart-yuruyusu mimariyi runtime'dan one gecirdi, denge geri aliniyor). TECH-DEBT Acik Borc 5/5 SINIRDA — yeni borc oncesi kapatma.
...
0. ⬜ cost_object SEMA dilimi: budget_cost_objects (btce-bazli, muhasebe-only RLS) + budget_items.cost_object_id nullable + restrict-silme + B19/updated_at kapsami + baseline snapshot kapsami. DDL onayi. 5-katman L1-L3.
0b. ⬜ vat_rate sema eki: budget_items.vat_rate (numeric default 20, >=0). Kucuk migration. supabase db push icin "kabul".
1. ⬜ BUTCE RUNTIME DIKEY-DILIMI (kart-yuruyusunden ONCELIKLI): fn_open_budget (raftan fotokopi: stages + cards + items[unit_net=0] + percent_lines + paket->item_burdens gunun orani + "Donemsiz" etabi + department_code->department_id cozumu; kopru bos) + TEK kartin CALISAN giris UI'i (kalem gir: ad/net/adet/birim/cost_object + canli toplam, DB'ye bagli) + ongorulen okuma (kopru + CFE -> matris). Tum yigini gercek kartta dogrular. Arada: sablon icerik seed + gorsel tasarim turu.
...
## Acik (kararlasmadi)
- department_code -> department_id cozumu: fn_open_budget'ta; kanonik departman kodu/seed gerekebilir.
