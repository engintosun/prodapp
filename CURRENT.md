...
- **6 Eylül 2026 — Üretim Kayıtları durağı canlıya girdi.** Bütçe rayında Bütçe Girişi'nden hemen sonra yeni durak açıldı (`nav-rail.tsx`, `authenticated-shell.tsx`); kendi ekranı (`production-records-screen.tsx`) masa + Oyuncular listesi olarak kuruldu, listeye giriş buradan yapılıyor. Karttan açılan Oyuncular panosu (`person-list-sheet.tsx`) SALT OKUNUR oldu, yalnız ajans/menajer tikleri karttan işaretlenebilir kaldı — `card-table-screen.tsx`'teki "+ Kişi ekle" ve çağıran taraf temizlendi. Şema dilimi (`20260906120000_uretim_kayitlari_temsilci_haneleri.sql`: `budget_cost_objects`'e ajans/menajer haneleri) Engin onayıyla CANLIYA ALINDI; dört kolon ve kısıt canlı DB'de salt-okuma sorguyla doğrulandı. Ekranda gerçek veriyle elle deneme aynı gün YAPILDI ve DOĞRULANDI (bkz. Durum). Servis (`person-label-service.ts`) dört yeni hane ve `countPersonLabels` ile genişledi. Test: 328 → 330. Karar evi: `docs/KABUK-KARARLARI.md` §3/§12.2, `docs/butce/BUTCE-EKRAN-KARARLARI.md` §20. Aynı gün ikinci dilim: kart tarafında kişi seçimi Görev hanesine göre süzülür (afce76c) — yön tek taraflı, kart listeden okur. Satırdaki kişi simgesi artık seçili kişinin adını ya da "Kişi seç" gösteriyor (5 Eylül keşfedilebilirlik bulgusunun ucuz yarısı). Test: 330 → 337.

## Durum

- HEAD: afce76c (6 Eylül 2026 — kart tarafında kişi seçimi görev hanesine göre süzülür). Denetim E gereği burada CURRENT.md'ye dokunan son commit'in EBEVEYNİ yazar; kapanışta YAZMA ANINDAKİ HEAD yazılır, çünkü kapanış commit'i CURRENT.md'ye dokunur ve "son commit" o olur.
- MUHUR-3 CANLIDA ve doğrulandı. Kişi ve iş etiketleri artık PROJE kapsamında.
- ÜRETİM KAYITLARI DURAĞI kod VE şema CANLIDA (bkz. Milestone 6 Eylül). Şema (ajans/menajer haneleri) `db push` ile uygulandı, dört kolon + `budget_cost_objects_temsilci_haneleri_check` kısıtı salt-okuma sorguyla doğrulandı. Üretim Kayıtları durağı, ajans/menajer haneleri ve görev süzmesi tarayıcıda gerçek projede elle denendi ve DOĞRULANDI (6 Eylül 2026).
- Migration 20260901130000'den 20260906120000'e kadar (KART 1600 M1/M2/M3, K-B iptali, MÜHÜR-3 dilim 1-2, Üretim Kayıtları temsilci haneleri dahil) CANLIDA. Kod: 337/337 test. Build geçer, eslint 0 hata. Originde tek dal: main.
...
**MADDE 1: ÜRETİM KAYITLARI LİSTESİNDE TABLO KIPIRDAMASI.** Ajans ya da menajer tiki işaretlenince ad hanesi doğuyor ve tablonun tamamı daralıyor — Rol, Oyuncu, Görev sütunlarının üçü birden küçülüyor, dokunulmamış satırlar da yerinden oynuyor (Engin ekran denemesi, 6 Eylül 2026). Sebep: tablo genişliğini içeriğe bırakıyor. Çözüm yönü: sütun genişlikleri sabitlenir, ad hanesinin yeri tik atılmadan da ayrılmış olur. Tek dosya: `production-records-screen.tsx`.

**MADDE 2: TEMSİLCİ KOMİSYONU SATIRININ DOĞUMU.** Bugün ajans/menajer tiki YALNIZ VERİ (6 Eylül kararı). Satırın doğması için önce iki soru kapanmalı: (a) satır hangi bütçeye doğacak — aynı projede birden çok bütçe var ve Üretim Kayıtları bütçeyi görmüyor; (b) varsayılan oran nereden gelecek — `item_library`'de böyle bir hane YOK (BUTCE-SEMA-KARARLARI "VARSAYILAN ORAN item_library'DE YOK", 5 Eylül 2026).
...
- Kadroda bir görevden kaç kişi var, kartta o görevden kaç satır var — farkın kullanıcıya söylenip söylenmeyeceği. 6 Eylül 2026'da kapsam dışı bırakıldı, karara bağlanmadı.
