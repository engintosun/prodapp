...
- 31 Ağustos 2026 (ayrı bir tur): statü kolonu 128'den 106'ya indirildi (ölçüm: Windows + Chrome, gerçek pencere, "Konaklama" = 67,84px, select ihtiyacı 105px); statü hücresi selectTd'ye geçti, çift dolgu kalktı; tableMinWidth 1348. Mac'te DOĞRULANMADI.
- 31 Ağustos 2026 (ayrı bir tur): bordro motorunun DB okuması toplu hale getirildi (`deriveBordroFieldsBatch`); kart açılışında kalem başına altı sorgu yerine bütçenin tamamı için altı sorgu; montaj saf fonksiyona (`assembleBordroInput`) çıkarıldı ve test edildi; kullanıcıya görünen davranış DEĞİŞMEDİ.
- 31 Ağustos 2026 (ayrı bir tur): kart kapağındaki rakam Maliyet'e geçti; masa artık kart tablosuyla aynı hesabı (`cardTotals`) kullanıyor; `kartNetToplamlari` ve `fetchCardNetTotals` silindi; getCard'in kalem eşlemesi `mapItemRows`'a çıkarıldı ve iki yüzey de onu kullanıyor.
