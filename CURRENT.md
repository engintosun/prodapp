...
## Bu prompt'ta yapildi (2026-07-10)
VERGI-MEVZUATI §3 itemize+tekstil-giyim eklendi; Konaklama option etiketi "Konaklama/Yemek" oldu; statü sütun başlığına "?" ikonu + 6 statülük referans bottom-sheet eklendi; SNL-TAKVIM-VARSAYILAN sinyali (payroll.ts union tipi + K7 varsayım dalında signals.push + budget-service.ts anchorOf kablolaması + UI notu + 4 yeni test); EKRAN-MUHASEBE §18 (Kurulum Modu 5 adıma çıktı, Şirket Tanımı ilk adım) + §19 (Tanımlar iki bölüm + oran yazmama kuralı) revizyonu. Build ✓, tüm testler ✓ (63 mevcut + yeni testler).
...
2. **ŞİRKET PROFİLİ ŞEMA DİLİMİ (ayrı prompt bekliyor):** user_id 1:1 tablo (company_name, legal_type, 3 boolean, updated_at); RLS — davete gelen muhasebe kullanıcısı proje sahibinin (projects.created_by) profilini günceller, kendi boş profilini AÇMAZ; göç — mevcut her kullanıcı için en son projesindeki company_name ile tek satır; Proje Aç davranışı — profil yoksa yazılan isim profili oluşturur, varsa dolu gelir; profil değişince mühürsüz projelerde bordro iskelet yükleri yeniden doldurulur (fn_refill benzeri tetik); Kurulum Modu 1. adım UI.
