...
- HEAD: c561ebd (31 Ağustos 2026 — kolon modeli, statü ayrımı, kapak Maliyet). Denetim E gereği burada CURRENT.md'ye dokunan son commit'in EBEVEYNİ yazar; kapanışta YAZMA ANINDAKİ HEAD yazılır, çünkü kapanış commit'i CURRENT.md'ye dokunur ve "son commit" o olur.
...
- Bütçe kolon modeli bugünkü hâli: 13 kolon — No · Ad · Statü · Dönemler · Birim · Birim net · Miktar · X · Ara toplam · Yasal Yük · Maliyet · KDV · Toplam. Ara toplam = çıplak çarpım, Yasal Yük = saf yük, Maliyet = Ara toplam + Yasal Yük, Toplam = Maliyet + KDV; tableMinWidth 1348, kullanılabilir alan ~1169 (fark DURUYOR, çözülmedi). Statü kümesi SEKİZ — `yemek` 31 Ağustos'ta `konaklama`dan ayrıldı, vergi rejimi ikisinde aynı, ayrım anlam ayrımı. Statü kolonu 106px'e indirildi (ölçüm: Windows + Chrome gerçek pencere; MAC'TE DOĞRULANMADI; ölçüm aracı `scripts/statu-genislik-olcumu.html`). Bordro motorunun DB okuması TOPLU (`deriveBordroFieldsBatch`); kart açılışında kalem başına değil bütçe başına altı sorgu; `deriveBordroFields` artık onun ince sarmalayıcısı, iki fetch yolu YOK. Kart kapağındaki rakam MALIYET ve kart tablosunun toplam şeridiyle AYNI fonksiyondan (totals.ts cardTotals) geliyor; `kartNetToplamlari` ve `fetchCardNetTotals` SİLİNDİ, ikinci tanım kalmadı.
...
1. **KOLON HİZASI VE TUTAR KOLONU GENİŞLİĞİ TURU.** Neye bağlı: hiçbir şey. Ekrandan tespit edildi (31 Ağustos 2026, Engin) ve dört belirti ÜÇ köke iniyor: (i) Silme "×" işareti son hücrenin İÇİNDE yaşıyor; "×"i olmayan satırlar o yeri kullanıyor ve rakamlar sağa kayıyor (Engin'in 1. ve 3. maddeleri AYNI köktür). (ii) Birim hücresi çok dönemli kalemde düz metin, tek dönemlide açılır liste; ikisinin dolgusu farklı — statüde 31 Ağustos'ta çözülen çift-dolgu kusurunun AYNI SINIFI. (iii) KDV ile Toplam rakamları BİRBİRİNE GİRİYOR — bu hiza değil GENİŞLİK sorunudur; tutar kolonlarının 112px'i "999.999.999,99 = 92,1px" ölçümüne dayanıyor, gerçek veride 152.223.822,98 gibi daha uzun rakamlar var. Tur, statüde yapılanın aynısıyla başlamalı: önce ölç, sonra daralt/genişlet. Ölçüm aracı zaten kurulu (`scripts/statu-genislik-olcumu.html`).
