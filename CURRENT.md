...
- 2026-06-25: DILIM-2d CANLI (commit e9dfe58): KART 1500 ekran Net + Brut + KDV ayri kolon; Yasal Yuk = brut-net (TL); kova cinsi servis->UI borusu; bottom-sheet para selalesi (Net -> +KDV = Kisiye banka odemesi / Yasal Yuk -> Brut). CFE kisiyeBanka eklendi (25 test yesil, container'da dogrulandi).
SIRADAKI: UI duzeltmeleri (m2,m3,m5+m6) -> DILIM-3 bordro motoru -> diger kartlar (1100/1300/1400/1600).
...
- YUK KOVASI CINS CANLI (2a-2d): burden_components.kind (additive/deduction); payment_status_burdens eslemesi (smm/kira->stopaj, telif->stopaj_telif; sirket/konaklama bos; bordro motor bekliyor); rate_catalog tek oran evi (stopaj 20, stopaj_telif 17, ... TASLAK). fn_open_budget statuye gore item_burdens snapshot. Servis kovayi kind ile ceker; CFE cinse gore brut (additive x(1+SUM) / deduction /(1-SUM)).
...
- YAPILDI (2a-2d): Kalem + Aciklama + Statu (6 deger native dropdown) + Donemler (tikla-ekle + donem-net override) + KDV (oran input) + Yasal Yuk (brut-net TL, tikla->bottom-sheet) + Net toplam + Brut toplam. Sema cins + statu-esleme + statu-fill CANLI; CFE cinse gore brut/net; selale dokum (Net->+KDV=Kisiye banka / Yasal Yuk->Brut).
...
- Statu->KDV varsayilan tutarlilik (kira sec -> vat 0 oto): payment_status_defaults.default_vat_rate cetvelinden; kucuk is, 2d disi (istege bagli).
