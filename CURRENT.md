...
2. Sifre/token rotasyonu — Engin'in kendi yapacagi dis is, birkac kez ertelendi, hala bekliyor.
3. 4a — KART 1500 isim onarimi (kapsam netlesmedi).
4. 4b — Kalem Kutuphanesi/Kalibrasyon (DILIM-4, ayri, Engin'in kuratorluk isi bekleniyor).
5. Butce Giris/Acilis Ekrani dilimi — bu, session basindan beri bekleyen asil siradaki is (tarih girisi + SGK senaryo secimi + tesvik bilgilendirmesi + Ocak-varsayimi aciklamasi). DILIM-3e onun onune gecti, hic baslanmadi.
...
- Şirket-Profili SGK senaryo SEÇİMİ altyapısı YOK (proje/şirket düzeyi kolon+ekran); 3b'de yalnız 4 senaryo kataloğa girdi (sgk_isveren=standart KOVAYA bağlı/varsayılan; borçlu/kültür-girişim/kültür-yatırım katalogda REFERANS/bekliyor). Kurulma zamanı: Butce Giris/Acilis Ekrani dilimi (bkz. Siradaki is #5) VEYA parametre paneli ekranı (soru 5) açıldığında — hangisi önce gelirse. (deriveBordroFields bu senaryo secimini item_burdens'inda bulursa kullanir, bulamazsa [her zaman, bugun] rate_catalog standart degerine duser — senaryo mantiginin kendisini bilmez.)
