...
`src/app/muhasebe/budget/hooks/grid-navigation-core.ts` (422)
...
`src/app/muhasebe/budget/card-table-screen.tsx` (1086)
...
`src/app/muhasebe/budget/card-view.ts` (166) — kartın görünen düzeninin (başlık grubu + kişi bloğu + türetilen satır tutarları) TEK kaynağı; dışa aktarma/icmal/mühür de bunu çağıracak
...
`src/app/muhasebe/budget/components/bottom-sheet.tsx` (237) — ortak alt-sheet primitivi (backdrop+panel+odak tuzağı)
...
`src/app/muhasebe/budget/sheet-placement.ts` (37) — tetiğin yanında açılan pencerenin yerleşim hesabı (alta ya da üste açılma, sola kayma, boy sınırı); saf işlev, ortak pencere parçası (bottom-sheet.tsx) çağırır
...
| Uygulama kabuğu (dört bölge, sol ray, üst bağlam, modül kapıları, adres şeması) · üç katman (şirket ayarı / şirket kütüphanesi / proje dökümü) · döküm evi · kişi listesi geçiş kuralı | `docs/KABUK-KARARLARI.md` |
...
Test sayıları `npm test` çıktısından ÖLÇÜLEREK okundu (24 Eylül 2026), toplam 452/452 geçti (24 dosya).
...
- `src/app/muhasebe/budget/display-name.test.ts` — itemDisplayName/summaryDisplayName'i (AD YERLEŞİMİ kararının saf hesabı) korur — 16 test
- `src/app/muhasebe/budget/person-bring.test.ts` — personCardPresence/personNameCollisions/sortPersonsByDuty/cardUsesPersonList'i (Oyuncular listesi getirme mantığı ve düğmenin çizilmesi) korur — 13 test
...
- `src/app/muhasebe/budget/sheet-placement.test.ts` — placeSheet'i (tetiğin yanında açılan pencerenin alta ya da üste yerleşimi, sola kayma, dar ekran) korur — 9 test
