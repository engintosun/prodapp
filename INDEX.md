...
`src/app/muhasebe/budget/components/burden-sheet.tsx` (105) — Yasal Yük dökümü sheet'i (bordro 6-bacak + basit statü)
...
`src/app/muhasebe/budget/components/note-sheet.tsx` (37) — İç Not / Kamu Notu düzenleme sheet'i
...
`src/app/muhasebe/budget/sheet-placement.ts` (34) — tetiğin yanında açılan pencerenin yerleşim hesabı (alta ya da üste açılma, sola kayma, boy sınırı); saf işlev, ortak pencere parçası (bottom-sheet.tsx) çağırır
...
Test sayıları `npm test` çıktısından ÖLÇÜLEREK okundu (24 Eylül 2026), toplam 443/443 geçti (24 dosya).
...
- `src/app/muhasebe/budget/sheet-placement.test.ts` — placeSheet'i (tetiğin yanında açılan pencerenin alta ya da üste yerleşimi, sola kayma, dar ekran) korur — 7 test
