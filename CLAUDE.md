...
## Doğrulama
- **Push doğrulama (zorunlu — Engin kararı 2026-08-01):** Opus, Sonnet'in raporuna GÜVENMEDEN her push'u origin'den bağımsız doğrular: hash eşitliği (HEAD = origin/main), dal adı, `git ls-remote --heads` ile başıboş dal taraması, commit'te değişen dosya listesi. Rapor doğrulama DEĞİLDİR — "main'e gitti / kapılar yeşil" ancak doğrulandıktan sonra denir. Doğrulama `git ls-remote` veya sığ fetch ile yapılır, tam klon gerektirmez; "rutin elle klonlama yok" ve araç orantısı kuralları bu görevi KAPSAMAZ.
