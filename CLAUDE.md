...
- **Engin onayı:** SADECE şema, RLS ya da grant değişikliği UYGULANMADAN ÖNCE (canlı KVKK verisi güvenlik geçidi). Onay = SQL'i okuyup "kabul" demek; kopyala yapıştır yok, uygulamayı Sonnet yapar. Kod ve normal işler onay gerektirmez, salt okuma işlemler onay gerektirmez. Bu kapı `.claude/settings.json` içinde `ask` kuralı olarak da kuruludur. **22 Eylül 2026:** kural yalnız `supabase` ile başlayan komutu tutuyordu, çalışan komut `npx supabase db push` olduğu için izin ekranı hiç çıkmıyordu (ölçüldü). `npx` biçimi eklendi ve kuru denemede (`npx supabase db push --dry-run`) izin ekranının çıktığı doğrulandı.
