-- D3-ARA: Aciklama kolonu semasi + name_en yeniden adlandirma (TD-20 kapanisi)
-- Davranis-sifir: ekranda hicbir degisiklik yok, UI kolonu bir sonraki dilimde (D3-UI).
-- Icerik: budget_items.description (yeni, bos dogar) + description_en -> name_en
--         (budget_items + item_library) + sema yorumlari + fn_open_budget/fn_add_budget_item
--         yeniden yaratma (INSERT hedef kolonu + degisken adlari).

alter table public.budget_items add column description text;

comment on column public.budget_items.description is 'Kullanicinin serbest aciklamasi (UI: Aciklama kolonu). Bos dogar. Uzun metin buraya degil Not sheet''ine (internal_note/public_note) gider.';

alter table public.budget_items rename column description_en to name_en;
alter table public.item_library rename column description_en to name_en;

comment on column public.budget_items.name is 'Kalem adi (UI: Ad kolonu). NOT: 20260613115009 icindeki "UI: Sebep" satir-ici yorumu GECERSIZDIR, o donemin kalintisidir.';
comment on column public.budget_items.name_en is 'Sablonun Ingilizce adi. Arka planda durur, ekranda hicbir kolonda gorunmez. Sablon govdesindeki JSON anahtari hala "detail" — hizalanmadi, TD-21.';
comment on column public.item_library.name_en is 'Katalog kaleminin Ingilizce adi. Arka planda durur.';

-- fn_open_budget: taban 20260723130000 (D2) surumudur. Delta yalniz INSERT hedef kolonu:
--   description_en -> name_en (v_item->>'detail' okumasi AYNEN kalir, JSON anahtari degismedi)
...
-- fn_add_budget_item: taban 20260723130000 (D2) surumudur. Delta: v_desc_en -> v_name_en,
--   v_lib.description_en -> v_lib.name_en, INSERT hedef kolonu description_en -> name_en.
--   description INSERT listesine EKLENMEZ (bilincli — yeni kalem NULL description ile dogar).
