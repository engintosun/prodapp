...
- **YAZI TİPİ VE PLATFORM TUTARLILIĞI (31 Ağustos 2026'da tespit edildi, karara BAĞLANMADI).** `--font-sans` değeri `system-ui, -apple-system, "Segoe UI", ...`; Windows'ta Segoe UI'a, Mac'te San Francisco'ya düşer. Bunlar farklı ölçülü yazı tipleridir, dolayısıyla table-styles.ts'teki ÖLÇÜLMÜŞ SABİTLERİN TAMAMI (92,1px · 67,84px · 105px · 112 · 106 · 1348 · kullanılabilir ~1169) yalnız Windows için geçerlidir. İkinci kalem: kaydırma çubuğu Windows'ta düzen genişliği alır, Mac'te üste biner — "kullanılabilir alan" ölçüsü de platforma bağlı. Üçüncü kalem: açılır liste okunun bedeli Windows + Chrome'da ÖLÇÜLDÜ, 20px (appearance:none ile aynı select 85px istiyordu, native 105px). Üç kademe konuşuldu: yazı tipini kendimiz taşımak (teknik iş küçük, ama uygulamanın TÜM tipografisi değişir — tasarım kararı), kaydırma çubuğu yerini sabitlemek (ucuz), form denetimlerini kendimiz çizmek (orta, ama yazı tipi durdukça sorunun küçük yarısını çözer). Zamanlama uyarısı: bedel yapılmadıkça büyür, çünkü her ölçüm turu repoya bir Windows sabiti daha çakar. Repoda Mac ÖNCELİKLİ platform olarak yazılı (18 Temmuz 2026).
