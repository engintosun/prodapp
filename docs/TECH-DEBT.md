...
|TD-20|budget_items'ta `name` ile `description_en` semantik olarak BİRBİRİNİN ADINI taşıyor: `description_en` kendi migration yorumuna göre "şablonun İngilizce ADINI" tutar (yani `name_en` olmalıydı), `name` ise ekranda "Açıklama" olarak görünür (yani `description` olmalıydı). Ayrıca `name` kolonunun şema yorumu hâlâ "UI: Sebep" diyor; canlı sonucu: kalem adı boş bırakılınca kullanıcı "Sebep boş olamaz" hatası görüyor ama ekranda "Sebep" diye bir kolon yok.|budget_items şeması + updateItemField hata metni|Kaza: Haziran'da UI "Sebep" derken kolon `name` kondu, UI "Açıklama"ya döndü kolon yerinde kaldı, 1 Temmuz'da `description` adı `description_en`'e kaptırıldı (o migration `detail` alanının iki iş yaptığını doğru teşhis edip ayırmıştı). 24 Temmuz tasarım oturumunda bulundu; yeni kolon açma önerisi reddedildiği için ACİL DEĞİL.|Yeniden adlandırma göçü mü, yalnız hata metni düzeltmesi mi?|24 Temmuz 2026|
...
- Karar Bekleyen: 5 (TD-2, TD-3, TD-6, TD-8, TD-19) — bütçeye sayılmaz
- **DURUM: 4/5.** BORC-A + BORC-B TAMAM. TD-14 kapandi (18 Temmuz 2026, KLV kapanis paketi) — TD-15/TD-16 ayni pakette acilip kapandi, hicbiri Acik Borc'a yansimadi. 19 Temmuz 2026: KLV manuel test turunda üç hata düzeltildi + TD-17 (hesaplama-seviyesi) ve TD-18 (Karar Bekleyen) açıldı, sınıra ulaşılmıştı (5/5). 20 Temmuz 2026: TD-17 kapandı (bordro motor mimarisine dokunulmadan servis-katmanı filtresiyle) — sınırın altına inildi. Aynı gün TD-18 de kapandı (asgari ücret alt sınır uyarısı + bölüm/sabit birim kaldırma); mesai/fazla-çalışma ihtiyacı TD-19 olarak Karar Bekleyen'e açıldı. 24 Temmuz 2026: TD-20 (name/description_en semantik takası + 'Sebep' hata metni) Karar Bekleyen'e açıldı — Açık Borç sayısı DEĞİŞMEDİ, 4/5.
