...
- **REFERANS** — oran/mevzuat cetvelleri (rate_catalog türetimi), salt-okunur. Geçerli SGK senaryosu işaretlenir (2026-09-18): işaret satırın İÇİNDEDİR, etiketin yanındadır (gerekçe: kutunun dışında duran yazı hangi satıra ait olduğunu söylemiyordu, tarayıcıda bir üstteki satıra ait sanıldı — 18 Eylül 2026); yazı "Bu şirkette geçerli"dir. Şirket Tanımı hiç kaydedilmemişse varsayım Standart'tır ve yazı bunu söyler: "Bu şirkette geçerli (Şirket Tanımı boş, varsayılan)". Senaryo çözümü bordro tarafındaki TEK yerde durur (`resolveSgkScenarioCode`, payroll-read.ts), ekran oradan okur — ikinci bir çözümleme yeri açılmaz. Boş profilde Standart seçilmesinin gerekçesi: borç durumu kullanıcının bugün bildiği bir şeydir, bilinmeyen değil sorulmamıştır; ihtiyat doktrini bilinemeyen için kurulmuştur. DEĞER YAZIMI CİNSTEN KARARLAŞIR (2026-09-18): satırın sonundaki birim eki, `rate_catalog` satırının CİNSİNDEN okunur — oranda yüzde işareti, tutarda TL, katsayıda EK YOK. Öncesinde ekran hangi kolonun dolu olduğuna bakıyordu ve tutar kolonunda duran SGK tavan katsayısına "9 TL" yazıyordu. Tek satıra "burada TL yazma" yaması YASAKTIR; yanlış olan yazım değil, cetvelin katsayıyı tutar sanmasıydı ve düzeltme cetvelde yapıldı (göç 20260918120000). Hesap `formatReferenceValue` (definitions-screen.tsx) tek yerinde durur.
