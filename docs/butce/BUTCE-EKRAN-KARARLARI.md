...
- GEDİK A (hücreler atlanıyor): Statü, Dönemler, Birim (native select) + Not ekle ve Yasal Yük (buton) motorun etiketini taşımıyor; motor Tab/ok basışında kendi haritasındaki bir sonraki etiketli hücreye ZORLA odaklanıyor (tarayıcının native Tab'ini preventDefault ile devre dışı bırakarak), bu beş hücre aradan hiç geçilmeden atlanıyor. Statü rehberi ("?") tetikleyicisi bu gediğe GİRMEZ — sayfa Tab sırasında zaten var, onun tek sorunu gedik B'dir.
  ...
  - Dönem satırındaki x (dönemi kaldır) butonu motora GİRMEZ, mouse-only kalır (Engin kararı, 2026-07-24) — tek tuşla geri alınamaz silme; "Silmek istediğinize emin misiniz?" benzeri bir onay adımı tasarlanana kadar klavyeye bağlanmaz.
...
- Durum: GEDİK 0 ve GEDİK B UYGULANDI (2026-07-24, bu commit); GEDİK A uygulaması ayrı spec'te bekliyor.
