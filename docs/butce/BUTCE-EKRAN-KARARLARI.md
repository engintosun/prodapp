...
**KARAR (güncel, 2026-07-27, D3-UI — Aciklama kolonu ayrildi):** Kolon sırası: **No · Ad · Açıklama · Statü · Dönemler · Birim · Birim net · Miktar · X · Yasal Yük · Net toplam · Brüt toplam** (12 kolon). Ad kolonu kalemin adını taşır (`budget_items.name`); Açıklama kolonu kullanıcının serbest kısa nitelik metnini taşır (`budget_items.description`), uzun metin Not sheet'ine gider. Eski `detail` alanı artık `name_en` alanında yaşıyor (Köster damıtımından gelen İngilizce standart adlar, şablonun İngilizce adını tutar, ileride İngilizce sunum için); ekranda hiçbir kolonda görünmez. Birim/Birim net sırası 2026-07-02'de değişti (Birim önce): sayısal kolonlar (Birim net'ten Brüt toplam'a) artık kesintisiz blok.
...
Not sheet'i acan isaret AD hucresinde kalir (Engin karari 2026-07-26), yeni Aciklama kolonuna gecmez; gerekce kalem kimliginin evi Ad hucresidir, sheet basligi da #kod + ad gosterir. Uzun metnin evi Not sheet'idir (Ic Not / Kamu Notu). Aciklama hucresi kisa nitelik icindir ("Yonetmen Kasesi — Ali Bey" gibi). Sinir asildiginda UYARI VERILMEZ, karakter siniri KONULMAZ (Engin karari 2026-07-26); gerekce: piksel sinirini olay olarak kullanmak cihaz genisligine gore degisir, ayni metin telefonda tasar genis ekranda tasmaz. Sinyal zaten var: odaksizken ellipsis + satirda duran Not isareti.
...
- Autocomplete: isim hücresine yazmaya başlayınca dropdown; kart açılışında o kartın kütüphane kalemleri bir kez çekilir, filtre istemci tarafında. **REVİZE (Engin kararı 2026-07-24): dropdown ODAKTA açılır, ilk tuşta değil** — kullanıcı Açıklama hücresine geldiğinde o kartın kütüphane kalemlerinin TAMAMINI görür (boş sorgu = süzme yok), isterse listeden seçer, isterse yazarak daraltır. Gerekçe: kart bağlamı zaten belli olduğu için liste kısa ve keşfedilebilir olmalı; "önce yaz, sonra gör" kullanıcıya kütüphanede ne olduğunu hiç göstermez. Türkçe-duyarsız eşleşme (İ/i, ı/I, ş/s) + İÇEREN-eşleşme ("asistan" → "Yönetmen Asistanı"). Liste YALNIZ aktif kartın kod aralığında arar, diğer kartların kalemleri GİZLİ (K-E) — uyarıya gerek yok çünkü aidiyet=kod doktrini altında "kodu başka, kartı bu" melez durumu tanımsız.
...
- Serbest kalem bilgi notu: NÖTR ton (bilgi ikonu, uyarı/kırmızı değil — serbest kalem meşru yol). Metin taslağı: "Bu kalem resmî kütüphanede yok; serbest kalem olarak eklendi ve kartın sonundaki Muhtelif bölümüne yerleşti. Statü ve birim seçimi size ait — yasal yükler bu seçime göre hesaplanır."
