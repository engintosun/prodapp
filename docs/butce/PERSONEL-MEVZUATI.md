...
- ✔ MÜHÜR (K7) — **Tarihsiz dönem:** çözücü varsayılan takvimi bütçenin açılış ayından DEĞİL, tarife yılının OCAK ayından kurar (ihtiyat-lehine risk payı — istisna serisinin en düşük değerleri, dolayısıyla maliyet en yüksek çıkar; Engin kararı 07-07, DILIM-3d kapanışı). İki özdeş bütçe, oluşturulma tarihinden bağımsız AYNI rakamı verir; gerçek tarih girildiğinde gerçek takvim kullanılır ve maliyet ancak aşağı iner. ŞART (Engin, aynen): normal Kaydet HİÇBİR koşulda tarih istemez ve engellemez; tarih zorunluluğu yalnız Mühür (Freeze/Lock) ve harcama fazına geçiş kapısında (`fn_lock_budget`). Şema hazır: `budget_stages.start_date/end_date` nullable + `is_undated`. EK ŞART (Engin, 2026-07-09): Bu iç takvim varsayımı kullanıcı arayüzünde hiçbir yerde takvim ay adıyla (Ocak, Şubat vb.) gösterilmez; bordro ekranlarında gerek yoksa ay göreli sayıyla bile gösterilmez — iç hesap parametre seçimi için kalır, kullanıcıya sızmaz.
