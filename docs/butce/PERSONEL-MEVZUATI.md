...
- ✔ **ZARF sözleşmesi:** net toplam · brüt toplam · kova dökümü · aylık seri (bordro ailesi: ay, kişi-başı brüt, SGK-işveren, GV, damga, istisna, kişi-başı maliyet, efektif X, ay toplamı) · sinyaller · parametre izi (kullanılan katalog satırlarının kimlikleri — Compliance Guard "kaynak-tarihli" ilkesinin teknik karşılığı). UI ve servis zarf dışını görmez.
...
- ✔ **Sinyal envanteri v0:** SNL-TAKVIM-VARSAYILAN (K7) · SNL-YIL-ASIMI (K5) · SNL-BOLUM-ESIT-DAGITIM (K8) · SNL-ADET-DEGISIM (K9) · SNL-EK6-GUN (ayın fiili gün sayısı verisiyle; 10-gün YORUMU uygunluk katmanının).
- ✔ MÜHÜR (K7) — **Tarihsiz dönem:** çözücü varsayılan takvimi bütçenin açılış ayından DEĞİL, tarife yılının OCAK ayından kurar (ihtiyat-lehine risk payı — istisna serisinin en düşük değerleri, dolayısıyla maliyet en yüksek çıkar; Engin kararı 07-07, DILIM-3d kapanışı). İki özdeş bütçe, oluşturulma tarihinden bağımsız AYNI rakamı verir; gerçek tarih girildiğinde gerçek takvim kullanılır ve maliyet ancak aşağı iner. ŞART (Engin, aynen): normal Kaydet HİÇBİR koşulda tarih istemez ve engellemez; tarih zorunluluğu YALNIZ harcama fazına geçiş kapısındadır; Mühür tarih İSTEMEZ — tarihsiz mühür Ocak-varsayımlı çözümü dondurur, mühür damgası sunumun "hazırlanma tarihi" referansıdır (REVİZE 2026-07-11, Engin kararı; sektör teyidi: MMB/Hot Budget/Saturation/Gorilla hiçbiri kilit için takvim istemez). Şema hazır: `budget_stages.start_date/end_date` nullable + `is_undated`. EK ŞART (Engin, 2026-07-09): Bu iç takvim varsayımı kullanıcı arayüzünde hiçbir yerde takvim ay adıyla (Ocak, Şubat vb.) gösterilmez; bordro ekranlarında gerek yoksa ay göreli sayıyla bile gösterilmez — iç hesap parametre seçimi için kalır, kullanıcıya sızmaz.
...
- ✔ MÜHÜR (K9) + **TERMİNOLOJİ MÜHRÜ:** Miktar = SÜRE (kaç gün/hafta/ay/bölüm) · X = kişi/adet SAYISI · Birim = periyodun CİNSİ (terminoloji tarihçesi: GLOSSARY). Kural: tek-kişi kümülatif zaman çizgisi pencere boyunca çözülür; her ayın kişi-başı maliyeti o ayın EFEKTİF X'iyle çarpılır. X dönemler arasında ARTARSA geç katılan kişi paylaşılan çizginin ilerlemiş kümülatifini devralır → vergi fazla tahmin edilir → sapma HER ZAMAN ihtiyat-lehinedir + SNL-ADET-DEGISIM.
- ⚠ **FAZ SINIRI — DİZİ KAYDI (Engin, 3 Temmuz 2026):** Tek-çizgi × X modeli (K9) ve yıl-aşımı-sıfırlamasızlığı (K5), reklam ve 2-3 aylık butik sinema projelerinde sorunsuzdur (sapma küçük). DİZİ (TV/Dijital) bütçeleme vizyonunda çalışma süreleri 6 ay ve üstüne çıkar; uzun pencerede her iki ihtiyat sapması BÜYÜR (geç katılan kişi modelde yüksek dilimden başlatılır; yılbaşı sıfırlaması kaçırılır). Dizi vizyonu tetiklendiğinde K9 ve K5 bu madde üzerinden BİRLİKTE yeniden açılır — ikisi de aynı "3-6 ay penceresi" varsayımına yaslanır. Aday mekanizma (kayıt, karar değil): kohort-bazlı çizgi — X artışında yeni kohort kendi kümülatifiyle başlar (kesin ve ucuz); X azalışında son-giren-ilk-çıkar konvansiyonu ihtiyat-lehinedir. CURRENT.md "KAPI AÇIK" listesine de işlendi.
...
- ✔ **Test/aldanmazlık katmanı:** altın fikstürler (K10 seti + Temmuz geçiş serisi) · round-trip property testi (rastgele net/ay/profil) · sınır testleri (dilim sınırına tam oturan matrah, tavan kesişimi, 1 günlük kıst, dönem ortası X artışı) · parametre-geçiş testi (Haziran→Temmuz). Hedef: 28 → ~50+ test.
...
### A. Ödeme statüleri ve katsayıları (kalem bazında)
...
- ● **Bordro (kadrolu):** basit katsayı YOK — kümülatif MOTOR (bölüm B). SGK biner, KDV yok.
...
- ✔ İLKE — **Net sabittir:** Birim net / Net toplam mevzuattan etkilenmez; kişiyle konuşulan rakam dokunulmazdır. Tüm bordro mevzuatı **Yasal Yük** kovasında yaşar. Net anlaşma nedeniyle hukuken "işçi kesintisi" olan kalemler de (SGK işçi %14 + işsizlik %1, GV, damga) ekonomik olarak yapımcıdadır — döküm tek liste halinde bileşen bazında gösterilir (SGK işçi, işsizlik işçi, gelir vergisi, damga, SGK işveren, işsizlik işveren — her biri kendi kalemiyle, kalem toplamı bazında, ay-ay değil), toplamın tamamı yüktür. Kişiye banka = net (bordroda KDV yok).
...
- ✔ KARAR — **Motor penceresi kalemin kendi verisinden türetilir:** dönem tarihleri (başlangıç ayı) + Birim + Miktar (süre) (terminoloji tarihçesi: GLOSSARY). Kullanıcıya ayrıca sorulmaz, saklanmaz (B18). Başlangıç ayı yalnız PARAMETRE seçimi için gerekir (istisna tutarı Temmuz'da, SGK taban/tavan Ocak'ta değişir). Gün/hafta birimli kalemler takvim ayına orantılı dağıtılır (kısmi ay kıst); kümülatif KİŞİ bazlı koşar, sonuç **X (kişi/adet)** ile çarpılır.
