...
- TEK HESAP İKİ YÜZEY: kapaktaki Maliyet, kart açılınca toplam şeridinde görünen Maliyet'in AYNISIDIR — artık aynı hesaptan (`card-view.ts` `cardViewTotals`) gelir, ikinci tanım yoktur. **DÜZELTME (24 Eylül 2026):** 9 Eylül 2026'dan beri kartın toplam şeridi `card-view.ts` hesabından (türetilen satır dahil) geliyordu, masa ise eski `totals.ts` `cardTotals` işlevinde kalmıştı; komisyon satırları masada sıfır sayılıyordu. Canlıda ölçüldü: Oyuncu kartında kapak 876.730,66, kartın içi 1.044.591,66, fark 167.861 ve bu fark komisyon satırlarının kart toplamının kendisi. Masa kartın hesabına bağlandı, `cardTotals` silindi. (Çapraz: BUTCE-EKRAN-KARARLARI bölüm 18.)
